  ...
  return (
    <Sheet open={isOpen} onOpenChange={setIsOpen}>
      <SheetTrigger asChild>
        <button className="relative p-2 text-foreground hover:text-primary transition-colors duration-200">
      ...
      <SheetContent className="w-full sm:max-w-lg flex flex-col h-full bg-background">
        <SheetHeader className="flex-shrink-0">
          <SheetTitle className="font-display text-xl">Καλάθι</SheetTitle>
          <SheetDescription>
            {totalItems === 0 ? "Το καλάθι σας είναι άδειο" : `${totalItems} προϊόν${totalItems !== 1 ? 'τα' : ''} στο καλάθι`}
          </SheetDescription>
        </SheetHeader>
            ...
            <>
                    ...
                    <div key={item.variantId} className="flex gap-4 p-3 bg-secondary/30 rounded-sm">
                      <div className="w-16 h-16 bg-secondary rounded-sm overflow-hidden flex-shrink-0">
                        {item.product.node.images?.edges?.[0]?.node && (
                          <img src={item.product.node.images.edges[0].node.url} alt={item.product.node.title} className="w-full h-full object-cover image-treatment" />
                        )}
                      </div>
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="font-medium text-sm mt-1">{parseFloat(item.price.amount).toFixed(2)}€</p>
                      </div>
              ...
              <div className="flex-shrink-0 space-y-4 pt-4 border-t bg-background">
                <div className="flex justify-between items-center">
                  <span className="font-display text-lg">Σύνολο</span>
                ...
                <Button onClick={handleCheckout} className="w-full bg-primary text-primary-foreground hover:bg-primary/90" size="lg" disabled={items.length === 0 || isLoading || isSyncing}>
                  {isLoading || isSyncing ? <Loader2 className="w-4 h-4 animate-spin" /> : <><ExternalLink className="w-4 h-4 mr-2" />Ολοκλήρωση Παραγγελίας</>}
                </Button>
              </div>
            </>
          )}
